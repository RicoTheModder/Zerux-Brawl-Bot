...
class TelegramBot:
    def __init__(self, token):
        ...
        self.user_config = load_user_config()        # user settings (ThemeID, etc.)
        ...
        self.usernames = {}          # chat_id -> Telegram @username
        ...
        # -------------------------
        # Unban Support Command (Admin Only)
        # -------------------------
        ...
        def unban_support(message):
            ...
            # Normalize input: remove any leading '@' and make lowercase
        ...
        def handle_support_message(msg):
            ...
            sender = self.get_formatted_username(msg.chat.id)
        ...
        def handle_reply(m):
            ...
            sender = self.get_formatted_username(m.chat.id)
            ...
            if user_chat_id:
                ...
                elif lower_reply.startswith("mute"):
                    parts = reply_text.split()
                    if len(parts) < 2 or not parts[1].isdigit():
                        self.bot.send_message(m.chat.id, "Invalid mute command. Use: Mute <minutes>")
                    else:
                        minutes = int(parts[1])
                ...
                elif lower_reply.startswith("ban"):
        ...
        def handle_news(msg):
            self.all_users.add(msg.chat.id)
            news_text = msg.text
            # Combine all known chat IDs (private chats, groups, channels, support group)
    ...
    # -------------------------
    def get_formatted_username(self, chat_id):
